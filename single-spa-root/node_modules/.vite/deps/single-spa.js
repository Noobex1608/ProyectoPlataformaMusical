...
function addProfileEntry(type, name, kind, start2, end, operationSucceeded) {
  profileEntries.push({
    type,
    name,
    start: start2,
...
function mountParcel(config, customProps) {
  ...
  loadPromise = loadPromise.then((config2) => {
    ...
    name = config2.name || `parcel-${id}`;
    if (
      // ES Module objects don't have the object prototype
      Object.prototype.hasOwnProperty.call(config2, "bootstrap") && !validLifecycleFn(config2.bootstrap)
    ) {
      throw Error(formatErrorMessage(9, `Parcel ${name} provided an invalid bootstrap function`, name));
    }
    if (!validLifecycleFn(config2.mount)) {
      throw Error(formatErrorMessage(10, `Parcel ${name} must have a valid mount function`, name));
    }
    if (!validLifecycleFn(config2.unmount)) {
      throw Error(formatErrorMessage(11, `Parcel ${name} must have a valid unmount function`, name));
    }
    if (config2.update && !validLifecycleFn(config2.update)) {
      throw Error(formatErrorMessage(12, `Parcel ${name} provided an invalid update function`, name));
    }
    const bootstrap = flattenFnArray(config2, "bootstrap");
    const mount = flattenFnArray(config2, "mount");
    const unmount = flattenFnArray(config2, "unmount");
    parcel.status = NOT_BOOTSTRAPPED;
    parcel.name = name;
    parcel.bootstrap = bootstrap;
    parcel.mount = mount;
    parcel.unmount = unmount;
    parcel.timeouts = ensureValidAppTimeouts(config2.timeouts);
    if (config2.update) {
      parcel.update = flattenFnArray(config2, "update");
  ...
  return Promise.resolve().then(() => {
    ...
    return appOrParcel.loadPromise = Promise.resolve().then(() => {
      ...
      if (isUserErr) {
        ...
        appOrParcel.loadErrorTime = (/* @__PURE__ */ new Date()).getTime();
      }
...
function getAppChanges() {
  const appsToUnload2 = [], appsToUnmount = [], appsToLoad = [], appsToMount = [];
...
function getAppStatus(appName) {
  const app = find(apps, (app2) => toName(app2) === appName);
...
function checkActivityFunctions(location2 = window.location) {
  return apps.filter((app) => app.activeWhen(location2)).map(toName);
}
...
function validateRegisterWithConfig(config) {
  ...
  const invalidKeys = Object.keys(config).reduce((invalidKeys2, prop) => validKeys.indexOf(prop) >= 0 ? invalidKeys2 : invalidKeys2.concat(prop), []);
...
var currentUrl = isInBrowser && window.location.href;
...
function reroute(pendingPromises = [], eventArguments, silentNavigation = false) {
  ...
  if (isStarted()) {
    ...
    appsThatChanged = appsToUnload2.concat(appsToLoad, appsToUnmount, appsToMount);
  ...
  function performAppChanges() {
    return Promise.resolve().then(() => {
      ...
      return Promise.all(cancelPromises).then((cancelValues) => {
        ...
        const unloadPromises = appsToUnload2.map(toUnloadPromise);
        ...
        const loadThenMountPromises = appsToLoad.map((app) => {
          return toLoadPromise(app).then((app2) => tryToBootstrapAndMount(app2, unmountAllPromise));
        });
...
function tryToBootstrapAndMount(app, unmountAllPromise) {
  if (shouldBeActive(app)) {
    return toBootstrapPromise(app).then((app2) => unmountAllPromise.then(() => shouldBeActive(app2) ? toMountPromise(app2) : app2));
  } else {
